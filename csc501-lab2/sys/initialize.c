...
extern void allocate_page_dir(int);
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	for(i = 0; i < 4; i++)
		...
		frm_tab[frame_number].fr_pid = NULLPROC;
		page_entry = (FRAME0+frame_number)*NBPG;
		for(j = 0; j < 1024; j++)
			{
			  page_entry->pt_pres = 1;	
  		      page_entry->pt_write = 1;
  		      page_entry->pt_user = 0;
  		      page_entry->pt_pwt = 0;
  		      page_entry->pt_pcd = 0;
  		      page_entry->pt_acc = 0;
  		      page_entry->pt_dirty = 0;
   		      page_entry->pt_mbz = 0;
   		      page_entry->pt_global = 1;
  		      page_entry->pt_avail = 0;
  		      page_entry->pt_base = i*FRAME0+j;	
  		      page_entry++;
			}
		}

	allocate_page_dir(NULLPROC);
